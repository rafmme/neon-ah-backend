...
import app from '../../..';
import {
  superAdminToken, userToken, userToken2, invalidToken
...
describe('Report Article Features', () => {
  describe('It should report articles', () => {
    it('Should return error for unauthorised user', async () => {
      const response = await chai
        .request(app)
        .post('/api/v1/articles/What-a-mighy-God/reports')
  ...
  describe('It should update reports', () => {
    ...
    it('Should return error for non existing articles', async () => {
      const response = await chai
        .request(app)
        .put('/api/v1/articles/What-a-mighy-God/reports')
        .set('Authorization', `Bearer ${userToken2}`)
        .send({
          complaint: 'too long'
        });
    ...
    it('should return error if author attempts to report own article', async () => {
      const response = await chai
        .request(app)
        .put('/api/v1/articles/how-to-be-a-10x-dev-sGNYfURm/reports')
        .set('Authorization', `Bearer ${userToken}`)
        .send({
          complaint: 'too long'
        });
    ...
    it('should successfully report article', async () => {
      ...
      expect(response.body.data.message).to.be.a('string');
      expect(response.body.data.message).to.be.eql(
        'Your complaint has been modified and will be acted on'
      );
      expect(response.body.data).to.have.property('statusCode');
      expect(response.body.data.statusCode).to.be.a('Number');
      expect(response.body.status).to.equal('success');
    ...
    it('should return error if article and user do not match', async () => {
      const response = await chai
        .request(app)
        .put('/api/v1/articles/how-to-say-hello-in-2019/reports')
        .set('Authorization', `Bearer ${userToken2}`)
        .send({
          complaint: 'too long'
        });
  ...
  describe('It should delete a report against a single article', () => {
    it('Should return error for unauthorised user', async () => {
      const response = await chai
        .request(app)
        .delete('/api/v1/articles/What-a-mighy-God/reports')
        .set('Authorization', `Bearer ${invalidToken}`)
        .send({
          complaint: 'too long'
        });
    ...
    it('Should return error for non existing report', async () => {
      const response = await chai
        .request(app)
        .delete('/api/v1/articles/how-to-say-hello-in-2019/reports')
        .set('Authorization', `Bearer ${userToken2}`)
        .send({
          complaint: 'too long'
        });
